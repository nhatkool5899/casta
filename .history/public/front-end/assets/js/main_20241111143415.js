...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var plays = document.getElementsByClassName('video__thumbnails');
    var videoCode = $('.code-video').val();
  
    if (btn) {
      
      btn.onclick = function () {
        iframe.src = videoSrc + videoCode + "?autoplay=1";
        modal.style.display = 'block';
      }
    }
    
    Array.from(plays).forEach(function (play) {
      play.onclick = function () {
        var youtubeId = play.getAttribute('data-youtube');
        var title = play.getAttribute('data-title');
        if (youtubeId) {
          $('.video-desc').text(title);
          iframe.src = videoSrc + youtubeId + "?autoplay=1";
          modal.style.display = 'block';
        }
      }
    });
