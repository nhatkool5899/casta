                ...
                <div class="card">
                    <div class="card-body">
                        <div class="box-title">
                            ...
                            <div><a href="{{route('customer.create')}}" class="btn-add"><i class="fa-solid fa-plus"></i> Thêm mới</a></div>
                        </div>
                        @if (session('status'))
                            ...
                            <tbody>
                                ...
                                @foreach ($customer as $item => $value)
                                    <tr>
                                        ...
                                        <td>{{$value->name}}</td>
                                        <td><img src="{{asset('uploads/customer/'.$value->image)}}" alt="" style="max-width:150px; background: #f2f2f2"></td>
                                        
                                        <td>
                                            <a href="{{route('customer.edit', ['customer' => $value->id])}}" class="edit-icon"><i class="fa-solid fa-pen"></i></a>

                                            <form action="{{route('customer.destroy',['customer' => $value->id])}}" method="post" style="display: inline-block">
                                                @method("DELETE")
                                                @csrf
